      ...
      <div className="hourlyContainer space-y-2 overflow-y-auto h-[560px] rounded-b-lg sm:pr-4 sm:pl-5 sm:py-5 p-5">
        {loading
          ...
          : groupedData[selectedDay]?.hours.map((hour, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between rounded-lg border border-theme bg-card-hover px-4 py-3 transition-colors hover:bg-card-hover hover:scale-103 ease-in-out"
                style={{ "--hover-bg": "var(--card-hover-color)" }}
                onMouseEnter={(e) =>
                  (e.target.style.backgroundColor = "var(--card-hover-color)")
                }
